...
{
	goop::sys::platform::vulkan::Context* ctx;
...
static uint32_t ImGui_ImplVulkan_MemoryType(VkMemoryPropertyFlags properties, uint32_t type_bits)
{
    ...
    vkGetPhysicalDeviceMemoryProperties(*bd->ctx, &prop);
...
static void CreateOrResizeBuffer(VkBuffer& buffer, VkDeviceMemory& buffer_memory, VkDeviceSize& p_buffer_size, size_t new_size, VkBufferUsageFlagBits usage)
{
    ...
    if (buffer != VK_NULL_HANDLE)
        vkDestroyBuffer(*bd->ctx, buffer, v->Allocator);
    if (buffer_memory != VK_NULL_HANDLE)
        vkFreeMemory(*bd->ctx, buffer_memory, v->Allocator);
    ...
    err = vkCreateBuffer(*bd->ctx, &buffer_info, v->Allocator, &buffer);
    check_vk_result(err);

    VkMemoryRequirements req;
    vkGetBufferMemoryRequirements(*bd->ctx, buffer, &req);
    bd->BufferMemoryAlignment = (bd->BufferMemoryAlignment > req.alignment) ? bd->BufferMemoryAlignment : req.alignment;
    VkMemoryAllocateInfo alloc_info = {};
    alloc_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    alloc_info.allocationSize = req.size;
    alloc_info.memoryTypeIndex = ImGui_ImplVulkan_MemoryType(VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, req.memoryTypeBits);
    err = vkAllocateMemory(*bd->ctx, &alloc_info, v->Allocator, &buffer_memory);
    check_vk_result(err);

    err = vkBindBufferMemory(*bd->ctx, buffer, buffer_memory, 0);
    check_vk_result(err);
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        ...
        VkResult err = vkMapMemory(*bd->ctx, rb->VertexBufferMemory, 0, rb->VertexBufferSize, 0, (void**)(&vtx_dst));
        check_vk_result(err);
        err = vkMapMemory(*bd->ctx, rb->IndexBufferMemory, 0, rb->IndexBufferSize, 0, (void**)(&idx_dst));
        check_vk_result(err);
        ...
        err = vkFlushMappedMemoryRanges(*bd->ctx, 2, range);
        check_vk_result(err);
        vkUnmapMemory(*bd->ctx, rb->VertexBufferMemory);
        vkUnmapMemory(*bd->ctx, rb->IndexBufferMemory);
    }
...
bool ImGui_ImplVulkan_CreateFontsTexture()
{
    ...
    {
        vkQueueWaitIdle(bd->ctx->getGraphicsQueue());
    ...
    if (bd->FontCommandPool == VK_NULL_HANDLE)
    {
        ...
        vkCreateCommandPool(*bd->ctx, &info, nullptr, &bd->FontCommandPool);
    }
    if (bd->FontCommandBuffer == VK_NULL_HANDLE)
    {
        ...
        err = vkAllocateCommandBuffers(*bd->ctx, &info, &bd->FontCommandBuffer);
        check_vk_result(err);
    }

    // Start command buffer
    {
        err = vkResetCommandPool(*bd->ctx, bd->FontCommandPool, 0);
        check_vk_result(err);
    ...
    // Create the Image:
    {
        ...
        err = vkCreateImage(*bd->ctx, &info, v->Allocator, &bd->FontImage);
        check_vk_result(err);
        VkMemoryRequirements req;
        vkGetImageMemoryRequirements(*bd->ctx, bd->FontImage, &req);
        VkMemoryAllocateInfo alloc_info = {};
        alloc_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        alloc_info.allocationSize = req.size;
        alloc_info.memoryTypeIndex = ImGui_ImplVulkan_MemoryType(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, req.memoryTypeBits);
        err = vkAllocateMemory(*bd->ctx, &alloc_info, v->Allocator, &bd->FontMemory);
        check_vk_result(err);
        err = vkBindImageMemory(*bd->ctx, bd->FontImage, bd->FontMemory, 0);
        check_vk_result(err);
    }
    ...
    // Create the Image View:
    {
        ...
        err = vkCreateImageView(*bd->ctx, &info, v->Allocator, &bd->FontView);
        check_vk_result(err);
    }
    ...
    VkBuffer upload_buffer;
    {
        ...
        buffer_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
        err = vkCreateBuffer(*bd->ctx, &buffer_info, v->Allocator, &upload_buffer);
        check_vk_result(err);
        VkMemoryRequirements req;
        vkGetBufferMemoryRequirements(*bd->ctx, upload_buffer, &req);
        bd->BufferMemoryAlignment = (bd->BufferMemoryAlignment > req.alignment) ? bd->BufferMemoryAlignment : req.alignment;
        VkMemoryAllocateInfo alloc_info = {};
        alloc_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        alloc_info.allocationSize = req.size;
        alloc_info.memoryTypeIndex = ImGui_ImplVulkan_MemoryType(VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, req.memoryTypeBits);
        err = vkAllocateMemory(*bd->ctx, &alloc_info, v->Allocator, &upload_buffer_memory);
        check_vk_result(err);
        err = vkBindBufferMemory(*bd->ctx, upload_buffer, upload_buffer_memory, 0);
        check_vk_result(err);
    }
    ...
    {
        char* map = nullptr;
        err = vkMapMemory(*bd->ctx, upload_buffer_memory, 0, upload_size, 0, (void**)(&map));
        check_vk_result(err);
        ...
        err = vkFlushMappedMemoryRanges(*bd->ctx, 1, range);
        check_vk_result(err);
        vkUnmapMemory(*bd->ctx, upload_buffer_memory);
    }
    ...
    check_vk_result(err);
    err = vkQueueSubmit(bd->ctx->getGraphicsQueue(), 1, &end_info, VK_NULL_HANDLE);
    check_vk_result(err);

    err = vkDeviceWaitIdle(*bd->ctx);
    check_vk_result(err);

    vkDestroyBuffer(*bd->ctx, upload_buffer, v->Allocator);
    vkFreeMemory(*bd->ctx, upload_buffer_memory, v->Allocator);
...
void ImGui_ImplVulkan_DestroyFontsTexture()
{
    ...
    if (bd->FontView)   { vkDestroyImageView(*bd->ctx, bd->FontView, v->Allocator); bd->FontView = VK_NULL_HANDLE; }
    if (bd->FontImage)  { vkDestroyImage(*bd->ctx, bd->FontImage, v->Allocator); bd->FontImage = VK_NULL_HANDLE; }
    if (bd->FontMemory) { vkFreeMemory(*bd->ctx, bd->FontMemory, v->Allocator); bd->FontMemory = VK_NULL_HANDLE; }
}
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    if (!bd->FontSampler)
    {
        ...
        err = vkCreateSampler(*bd->ctx, &info, v->Allocator, &bd->FontSampler);
        check_vk_result(err);
    }
    ...
    if (!bd->DescriptorSetLayout)
    {
        ...
        err = vkCreateDescriptorSetLayout(*bd->ctx, &info, v->Allocator, &bd->DescriptorSetLayout);
        check_vk_result(err);
    }
    ...
    if (!bd->PipelineLayout)
    {
        ...
        err = vkCreatePipelineLayout(*bd->ctx, &layout_info, v->Allocator, &bd->PipelineLayout);
        check_vk_result(err);
    }

    ImGui_ImplVulkan_CreatePipeline(*bd->ctx, v->Allocator, v->PipelineCache, bd->RenderPass, v->MSAASamples, &bd->Pipeline, bd->Subpass);
...
void    ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    ImGui_ImplVulkanH_DestroyAllViewportsRenderBuffers(*bd->ctx, v->Allocator);
    ImGui_ImplVulkan_DestroyFontsTexture();

    if (bd->FontCommandPool)      { vkDestroyCommandPool(*bd->ctx, bd->FontCommandPool, v->Allocator); bd->FontCommandPool = VK_NULL_HANDLE; }
    if (bd->ShaderModuleVert)     { vkDestroyShaderModule(*bd->ctx, bd->ShaderModuleVert, v->Allocator); bd->ShaderModuleVert = VK_NULL_HANDLE; }
    if (bd->ShaderModuleFrag)     { vkDestroyShaderModule(*bd->ctx, bd->ShaderModuleFrag, v->Allocator); bd->ShaderModuleFrag = VK_NULL_HANDLE; }
    if (bd->FontSampler)          { vkDestroySampler(*bd->ctx, bd->FontSampler, v->Allocator); bd->FontSampler = VK_NULL_HANDLE; }
    if (bd->DescriptorSetLayout)  { vkDestroyDescriptorSetLayout(*bd->ctx, bd->DescriptorSetLayout, v->Allocator); bd->DescriptorSetLayout = VK_NULL_HANDLE; }
    if (bd->PipelineLayout)       { vkDestroyPipelineLayout(*bd->ctx, bd->PipelineLayout, v->Allocator); bd->PipelineLayout = VK_NULL_HANDLE; }
    if (bd->Pipeline)             { vkDestroyPipeline(*bd->ctx, bd->Pipeline, v->Allocator); bd->Pipeline = VK_NULL_HANDLE; }
}
...
void ImGui_ImplVulkan_SetMinImageCount(uint32_t min_image_count)
{
    ...
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    VkResult err = vkDeviceWaitIdle(*bd->ctx);
    check_vk_result(err);
    ImGui_ImplVulkanH_DestroyAllViewportsRenderBuffers(*bd->ctx, v->Allocator);
...
VkDescriptorSet ImGui_ImplVulkan_AddTexture(VkSampler sampler, VkImageView image_view, VkImageLayout image_layout)
{
    ...
    VkDescriptorSet descriptor_set;
    {
        ...
        VkResult err = vkAllocateDescriptorSets(*bd->ctx, &alloc_info, &descriptor_set);
        check_vk_result(err);
    }
    ...
    // Update the Descriptor Set:
    {
        ...
        vkUpdateDescriptorSets(*bd->ctx, 1, write_desc, 0, nullptr);
    }
...
void ImGui_ImplVulkan_RemoveTexture(VkDescriptorSet descriptor_set)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    vkFreeDescriptorSets(*bd->ctx, v->DescriptorPool, 1, &descriptor_set);
}
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport)
{
    ...
    // Create surface
    ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
    VkResult err = (VkResult)platform_io.Platform_CreateVkSurface(viewport, (ImU64)(bd->ctx->getInstance()), (const void*)v->Allocator, (ImU64*)&wd->Surface);
    check_vk_result(err);

    // Check for WSI support
    VkBool32 res;
    vkGetPhysicalDeviceSurfaceSupportKHR(*bd->ctx, v->QueueFamily, wd->Surface, &res);
    if (res != VK_TRUE)
    {
    ...
    wd->SurfaceFormat = ImGui_ImplVulkanH_SelectSurfaceFormat(*bd->ctx, wd->Surface, requestSurfaceImageFormat, (size_t)IM_ARRAYSIZE(requestSurfaceImageFormat), requestSurfaceColorSpace);

    // Select Present Mode
    // FIXME-VULKAN: Even thought mailbox seems to get us maximum framerate with a single window, it halves framerate with a second window etc. (w/ Nvidia and SDK 1.82.1)
    VkPresentModeKHR present_modes[] = { VK_PRESENT_MODE_MAILBOX_KHR, VK_PRESENT_MODE_IMMEDIATE_KHR, VK_PRESENT_MODE_FIFO_KHR };
    wd->PresentMode = ImGui_ImplVulkanH_SelectPresentMode(*bd->ctx, wd->Surface, &present_modes[0], IM_ARRAYSIZE(present_modes));
    //printf("[vulkan] Secondary window selected PresentMode = %d\n", wd->PresentMode);

    // Create SwapChain, RenderPass, Framebuffer, etc.
    wd->ClearEnable = (viewport->Flags & ImGuiViewportFlags_NoRendererClear) ? false : true;
    wd->UseDynamicRendering = v->UseDynamicRendering;
    ImGui_ImplVulkanH_CreateOrResizeWindow(*bd->ctx, *bd->ctx, *bd->ctx, wd, v->QueueFamily, v->Allocator, (int)viewport->Size.x, (int)viewport->Size.y, v->MinImageCount);
    vd->WindowOwned = true;
}
...
static void ImGui_ImplVulkan_DestroyWindow(ImGuiViewport* viewport)
{
    ...
    if (ImGui_ImplVulkan_ViewportData* vd = (ImGui_ImplVulkan_ViewportData*)viewport->RendererUserData)
    {
        ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
        if (vd->WindowOwned)
            ImGui_ImplVulkanH_DestroyWindow(*bd->ctx, *bd->ctx, &vd->Window, v->Allocator);
        ImGui_ImplVulkanH_DestroyWindowRenderBuffers(*bd->ctx, &vd->RenderBuffers, v->Allocator);
        IM_DELETE(vd);
    }
...
static void ImGui_ImplVulkan_SetWindowSize(ImGuiViewport* viewport, ImVec2 size)
{
    ...
    ImGui_ImplVulkanH_CreateOrResizeWindow(*bd->ctx, *bd->ctx, *bd->ctx, &vd->Window, v->QueueFamily, v->Allocator, (int)size.x, (int)size.y, v->MinImageCount);
}
...
static void ImGui_ImplVulkan_RenderWindow(ImGuiViewport* viewport, void*)
{
    ...
    {
        {
          err = vkAcquireNextImageKHR(*bd->ctx, wd->Swapchain, UINT64_MAX, fsd->ImageAcquiredSemaphore, VK_NULL_HANDLE, &wd->FrameIndex);
          check_vk_result(err);
          fd = &wd->Frames[wd->FrameIndex];
        }
        for (;;)
        {
            err = vkWaitForFences(*bd->ctx, 1, &fd->Fence, VK_TRUE, 100);
            if (err == VK_SUCCESS) break;
            if (err == VK_TIMEOUT) continue;
            check_vk_result(err);
        }
        {
            err = vkResetCommandPool(*bd->ctx, fd->CommandPool, 0);
            check_vk_result(err);
...

    {
        ...
        }
        {
            ...
            check_vk_result(err);
            err = vkResetFences(*bd->ctx, 1, &fd->Fence);
            check_vk_result(err);
            err = vkQueueSubmit(bd->ctx->getGraphicsQueue(), 1, &info, fd->Fence);
            check_vk_result(err);
        }
    }
}
...
static void ImGui_ImplVulkan_SwapBuffers(ImGuiViewport* viewport, void*)
{
    ...
    err = vkQueuePresentKHR(bd->ctx->getGraphicsQueue(), &info);
    if (err == VK_ERROR_OUT_OF_DATE_KHR || err == VK_SUBOPTIMAL_KHR)
        ImGui_ImplVulkanH_CreateOrResizeWindow(*bd->ctx, *bd->ctx, *bd->ctx, &vd->Window, v->QueueFamily, v->Allocator, (int)viewport->Size.x, (int)viewport->Size.y, v->MinImageCount);
    else
        check_vk_result(err);
