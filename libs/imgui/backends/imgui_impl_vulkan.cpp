...
//  [x] Renderer: User texture binding. Use 'VkDescriptorSet' as ImTextureID. Read the FAQ about
//  ImTextureID! See https://github.com/ocornut/imgui/pull/914 for discussions. [X] Renderer: Large
//  meshes support (64k+ vertices) with 16-bit indices. [x] Renderer: Multi-viewport / platform
//  windows. With issues (flickering when creating a new viewport).
...
void ImGui_ImplVulkan_DestroyDeviceObjects()
{
	...
	if (bd->PipelineMultiView)
	{
		vkDestroyPipeline(v->Device, bd->PipelineMultiView, v->Allocator);
		bd->PipelineMultiView = VK_NULL_HANDLE;
	}
}
